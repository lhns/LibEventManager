...
public class Testclass {
    ...
    private Testclass(Event event) {
        event.addOutput(event.getEventType() + " testclass constructor priority="
    ...
    public static void main(String[] args) {
        ...
        for (Object obj : Events.eventmanager.callSync(Events.testevent1, 3).getOutput()) {
            System.out.println(obj);
        }
        for (Object obj : Events.eventmanager.callSync(Events.testevent2, 5).getOutput()) {
            System.out.println(obj);
        }
    }
    ...
    private static void staticlistener(Event event, Object randomobject, int someint) {
        event.addOutput(event.getEventType() + " testclass listener (static) (test1) priority="
                + event.getCurrentListenerContainer().getPriority());
    }

    @EventListener(value = "test1", filter = "filter2", priority = 1)
    public static void staticListener2(Event event) {
        event.addOutput(event.getEventType() + " testclass listener2 (static) (test1) priority="
                + event.getCurrentListenerContainer().getPriority());
    }
